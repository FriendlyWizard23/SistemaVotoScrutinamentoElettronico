...
public class RisultatoCategoricoPreferenza extends RisultatoGeneric {
	private List<Candidato> lc;
	private int totVotazioni;

	public RisultatoCategoricoPreferenza(SessioneVoto sv, List<Candidato> lc, int totVotazioni) {
		super(sv);
		this.lc = lc;
	...
	public String getVincitore() {
		...
		if (sv.getStrategiaVincitore().equals("maggioranza")) {
			...
			return ("CANDIDATO VINCITORE: " + vinc.getNome() + " " + vinc.getCognome() + " CON VOTI: "
					+ vinc.getTotVoti() + " DEL PARTITO: " + vinc.getPartito());
		} else {
			for (Candidato c : lc) {
