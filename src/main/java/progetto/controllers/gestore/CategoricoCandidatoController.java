...
public class CategoricoCandidatoController extends Controller {
	...
	@FXML
	private ScrollPane scrollCandidati;
	...
	private void loadCandidati() {
		List<Candidato> lc = sdi.getListCandidati();
		...
		for (Candidato c : lc) {
			...
			part.setUserData(c);
			part.setText(c.getNome() + " " + c.getCognome());
	...
	public void creaSessione() {
		...
		SessioneVoto nuovaSessione = new SessioneVoto(swg.getTocreate(), "categoricoCandidato", strategia, null, 0, 0,
				false, true);
		sdi.saveCategoricoCandidato(lst, nuovaSessione);
